...
import { BarChart2, Users, Activity, TrendingUp, Calendar, Download } from "lucide-react";
...
function xlAddSheetHeader(ws: any, wb: any, logoId: number | undefined, title: string, subtitle: string, oggi: string) {
  ...
  const r2 = ws.getRow(2); r2.getCell(2).value = title; r2.getCell(2).font = { bold: true, size: 10, color: { argb: XL_RED } };
...
function xlAddColHeaders(ws: any, headers: string[], fill = xlRedFill) {
  ...
  row.eachCell((cell: any, col: number) => {
    cell.fill = fill; cell.font = xlHeaderFont;
...
function xlAddDataRow(ws: any, data: any[], odd: boolean, redCols: number[] = []) {
  ...
  row.eachCell({ includeEmpty: true }, (cell: any, col: number) => {
    cell.fill = odd ? xlLightFill : xlWhiteFill; cell.border = xlThinBorder;
    cell.alignment = { vertical: "middle", horizontal: col === 1 ? "left" : "center" };
...
async function esportaExcelPanoramica(params: {
  ...
  [["Atleti totali in gestione", params.atleti.length], ["In riabilitazione", attivi], ["Guariti / Dimessi", guariti], ["Programmi riabilitativi creati", params.programmi.length]].forEach((r, i) => xlAddDataRow(ws1, r, i % 2 !== 0, [2]));
  ...
  params.perCategoria.forEach(({ cat, totale, attivi: a }, i) => xlAddDataRow(ws2, [cat, totale, a, totale - a], i % 2 !== 0, [2, 3, 4]));
...
async function esportaExcelReport(
  atletiMese: Atleta[], mese: number, anno: number, filtroCat: string, filtroTipoInf: string,
  _atleti: Atleta[], mesiP?: { anno: number; mese: number }[], periodoLbl?: string,
) {
  const { Workbook } = await import("exceljs");
  const wb = new Workbook();
  wb.creator = "U.S. Cremonese Rehab Area";
  ...
  const ggD = (inizio: string, fine?: string) => fine
  ...
  atletiMese.forEach((a, athleteIdx) => {
    ...
    const addRow = (values: any[]) => xlAddDataRow(ws, values, athleteIdx % 2 !== 0, [9]);
    if (infortuni.length === 0) {
      addRow([nd(a), a.categoria, "—", "—", "—", "—", "—", a.stato, `${a.progresso}%`]);
    } else {
      infortuni.forEach((inf) => {
        addRow([nd(a), a.categoria, inf.diagnosi, inf.tipo ?? "—", inf.inizio ? fmtD(inf.inizio) : "—", inf.fine ? fmtD(inf.fine) : "—", inf.inizio ? ggD(inf.inizio, inf.fine) : "—", a.stato, `${a.progresso}%`]);
      });
    }
    if (count > 1) {
      const endRowNum = startRowNum + count - 1;
      ws.mergeCells(startRowNum, 1, endRowNum, 1); ws.mergeCells(startRowNum, 2, endRowNum, 2);
...
export default function AnalisiPage() {
  ...
  const handleExport = async (tipo: "excel" | "csv") => {
    const key = tab + tipo;
    setEsportando(key);
    try {
      if (tab === "overview") {
        ...
        else esportaCSVPanoramica(params);
      } else {
        if (tipo === "excel") await esportaExcelReport(atletiMese, reportMese, reportAnno, filtroCat, filtroTipoInf !== "Tutti" ? filtroTipoInf : "", atleti, mesiPeriodo, periodoLabel);
        else esportaCSVReport(atletiMese, reportMese, reportAnno, filtroCat, filtroTipoInf !== "Tutti" ? filtroTipoInf : "", mesiPeriodo, periodoLabel);
      }
    } finally {
      setEsportando(null);
    }
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleExport("excel")} disabled={!!esportando}
              ...
              {esportando?.endsWith("excel") ? "..." : "Excel"}
            </button>
            <button onClick={() => handleExport("csv")} disabled={!!esportando}
              className="flex items-center gap-1.5 border border-gray-200 text-gray-600 px-3 py-2 rounded-xl text-xs font-semibold hover:bg-gray-50 disabled:opacity-50 transition-colors">
              <Download className="w-3.5 h-3.5" />
              {esportando?.endsWith("csv") ? "..." : "CSV"}
            </button>
          </div>
          {/* Tab switcher */}
