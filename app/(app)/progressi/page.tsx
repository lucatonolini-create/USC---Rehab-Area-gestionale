...
async function esportaExcelReportMensile(
  ...
  atletiMese.forEach((a, athleteIdx) => {
    ...
    const addXlRow = (values: any[]) => {
      ...
      row.height = 18;
      row.eachCell({ includeEmpty: true }, (cell: any) => { cell.fill = bg; cell.border = border; cell.font = { size: 9 }; cell.alignment = { vertical: "middle" }; });
    };
    if (infortuni.length === 0) {
      addXlRow([nd(a), a.categoria, "—", "—", "—", "—", "—", a.stato, `${a.progresso}%`]);
    } else {
      infortuni.forEach((inf) => {
        addXlRow([nd(a), a.categoria, inf.diagnosi, inf.tipo ?? "—", inf.inizio ? fmtD(inf.inizio) : "—", inf.fine ? fmtD(inf.fine) : "—", inf.inizio ? gg(inf.inizio, inf.fine) : "—", a.stato, `${a.progresso}%`]);
      });
    }
    if (count > 1) {
...
export default function ProgressiPage() {
  ...
  const [esportandoReport, setEsportandoReport] = useState<"excel" | null>(null);
  ...
  const handleExport = async (atleta: Atleta, tipo: "excel" | "csv") => {
                ...
                <div key={atleta.id} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                  <div className="flex items-start justify-between gap-4 mb-5 flex-wrap">
                    ...
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button onClick={() => handleExport(atleta, "csv")} disabled={!!esportando}
                        className="flex items-center gap-1.5 border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-gray-50 disabled:opacity-50">
                        <Download className="w-3.5 h-3.5" />
                        {esportando === atleta.id + "csv" ? "..." : "CSV"}
                      </button>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => { try { esportaCSVReportMensile(atletiMese, reportMese, reportAnno, filtroCat, mesiPeriodo, periodoLabel); } catch {} }}
                  disabled={atletiMese.length === 0}
                  className="flex items-center gap-1.5 border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-gray-50 disabled:opacity-50">
                  <Download className="w-3.5 h-3.5" />
                  CSV
                </button>
